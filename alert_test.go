...
func TestAlertListWithOptionalParams(t *testing.T) {
	...
	_, err := client.Alerts.List(context.TODO(), orb.AlertListParams{
...
func TestAlertDelete(t *testing.T) {
	...
	err := client.Alerts.Delete(context.TODO(), "alert_configuration_id")
	if err != nil {
		var apierr *orb.Error
		if errors.As(err, &apierr) {
			t.Log(string(apierr.DumpRequest(true)))
...
func TestAlertNewForCustomerWithOptionalParams(t *testing.T) {
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	}
...
func TestAlertNewForExternalCustomerWithOptionalParams(t *testing.T) {
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	}
