...
func TestInvoiceIssueSummaryWithOptionalParams(t *testing.T) {
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	}
	...
	_, err := client.Invoices.IssueSummary(
		context.TODO(),
		"invoice_id",
		orb.InvoiceIssueSummaryParams{
			Synchronous: orb.F(true),
		},
	)
	if err != nil {
...
func TestInvoiceRegenerateInvoicePdf(t *testing.T) {
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	}
	...
	_, err := client.Invoices.RegenerateInvoicePdf(context.TODO(), "invoice_id")
	if err != nil {
		var apierr *orb.Error
		if errors.As(err, &apierr) {
			t.Log(string(apierr.DumpRequest(true)))
...
func TestInvoiceRegenerateReceiptPdf(t *testing.T) {
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	}
	...
	_, err := client.Invoices.RegenerateReceiptPdf(context.TODO(), "invoice_id")
	if err != nil {
		var apierr *orb.Error
		if errors.As(err, &apierr) {
			t.Log(string(apierr.DumpRequest(true)))
