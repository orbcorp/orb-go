...
type EventUpdateResponse struct {
	...
	Amended string                  `json:"amended" api:"required"`
...
type EventDeprecateResponse struct {
	...
	Deprecated string                     `json:"deprecated" api:"required"`
...
type EventIngestResponseValidationFailed struct {
	...
	// idempotency_key.
	ValidationErrors []string                                `json:"validation_errors" api:"required"`
...
type EventSearchResponse struct {
	Data []EventSearchResponseData `json:"data" api:"required"`
...
type EventSearchResponseData struct {
	...
	Deprecated bool `json:"deprecated" api:"required"`
...
type EventIngestParams struct {
	Events param.Field[[]EventIngestParamsEvent] `json:"events" api:"required"`
...
type EventIngestParamsEvent struct {
	...
	// Exactly one event with a given idempotency key will be ingested, which allows
	// for safe request retries.
	IdempotencyKey param.Field[string] `json:"idempotency_key" api:"required"`
	// A dictionary of custom properties. Values in this dictionary must be numeric,
	// boolean, or strings. Nested dictionaries are disallowed.
	Properties param.Field[map[string]interface{}] `json:"properties" api:"required"`
	// An ISO 8601 format date with no timezone offset (i.e. UTC). This should
...
type EventSearchParams struct {
	...
	// sensitively.
	EventIDs param.Field[[]string] `json:"event_ids" api:"required"`
